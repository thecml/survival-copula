...
from data_loader import CompetingRiskSyntheticDataLoader, MultiEventSyntheticDataLoader
...
if __name__ == "__main__":
    dl = MultiEventSyntheticDataLoader().load_data(cfg.data_cfg, k_taus=[0.5, 0.5, 0.5],
                                                   copula_names=["clayton", "clayton", "clayton"],
    ...
    model = CopulaMLP(n_features, layers=layers, n_events=n_events,
                      n_dists=n_dists, copula=None, dgps=dgps,
                      time_bins=time_bins, device=device)
    model.fit(train_dict, valid_dict, lr_dict=lr_dict, n_epochs=n_epochs,
              patience=10, batch_size=batch_size, verbose=True, multi=True, weight_decay=0.01)
