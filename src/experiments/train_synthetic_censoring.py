...
if __name__ == "__main__":
    ...
    for seed in SEEDS:
        for copula_name in COPULA_NAMES:
            ...
            df = dl.get_data()
            censoring_rate = round(1.0 - df['event'].mean(), 2)
            
            print(f"Running: {seed}, {copula_name}, {k_tau}, {n_samples}, {n_features}, {censoring_rate}")
            ...
            # Make results
            results_dict[(seed, copula_name, k_tau, n_samples, n_features, censoring_rate)] = {} # tuple as key
    ...
    flattened_results = []
    for (seed, copula_name, k_tau, n_samples, n_features, censoring_rate), metrics in results_dict.items():
